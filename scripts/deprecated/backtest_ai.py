...
AI 분석을 "매매 판단(buy/sell/hold)" 에서 "정보 비서/브리핑(매매 권유 없음)" 으로
방향 전환하면서 이 백테스트 스크립트는 더 이상 의미가 없어졌습니다.
- 새 출력 스키마엔 action 필드가 없어 적중률 측정 자체가 불가
- 매매 권유를 안 하므로 "적중률" 이라는 개념이 부적절

보존 이유: 향후 재가동 가능성 또는 옛 로그(archive/ai_results/) 분석 참고용.
다시 사용하려면 scripts/ 로 옮기고 워크플로(.github/workflows/archive_ai.yml)에
재추가해야 합니다.
