...
# ⚠️ 2026-05-14 방향 전환: "AI 매매 판단" → "AI 정보 비서/브리핑(매매 권유 없음)"
# 이전 system 프롬프트는 action/confidence/analysis 출력을 요구했지만,
# 매매 권유로 해석될 위험을 제거하기 위해 정보 정리·요약·분류만 하도록 변경.
# Anthropic Tool Use 스키마 — 출력 형식 강제 (JSON 파싱 실패 방지)
AI_BRIEFING_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            "current_situation_summary": {
                "type": "object",
                "properties": {
                    "headline": {"type": "string", "description": "50자 내외 한 줄 사실 진술"},
                    "key_points": {
                        "type": "array",
                        "items": {"type": "string"},
                        "maxItems": 5,
                        "description": "현재 상황을 요약하는 핵심 사실 3~5개"
                    }
                },
            ...
            "factors_to_consider": {
                ...
                "items": {"type": "string"},
                "maxItems": 5,
                "description": "판단의 근거가 될 사실들. 매매 권유·행동 동사 절대 금지."
            },
            "user_position_notes": {
                "type": "array",
                "items": {"type": "string"},
...
def get_ai_analysis(code, client_ip=None, avg_price=None, shares=None):
    """AI 정보 비서 — 종목 브리핑 (매매 권유 아님).

    avg_price (선택): 사용자 평단가. 입력 시 user_position 섹션 채워짐.
    shares (선택):    보유 수량. avg_price 와 함께 입력 시 손익액(원) 도 계산.
    """
    ...
    try:
        ...
        shares_int = None
    # shares 만 입력하고 avg_price 없으면 무시
    ...
    if cached_raw:
        try:
            ...
            if isinstance(cached_result.get("metadata"), dict):
                ...
                cached_result["cached"] = True  # 옛 형식 호환
    ...
    # 사용자 평단/수량 섹션 (있을 때만 prompt에 추가)
    position_text = ""
    position_notes_instruction = "user_position_notes 는 평단 입력이 없으므로 비워두세요 (배열을 응답에서 생략)."
    if avg_price_int:
        ...
        position_notes_instruction = (
            "평단 입력이 있으므로 user_position_notes 에 사용자가 평단을 보고 *판단의 맥락이 될 사실* "
            "2~3개를 적어주세요 (예: '60일 최저 대비 +N% 위치', '20일 평균선 아래'). "
            "매도/추매/익절/손절 같은 행동 권유 절대 금지."
        )
...
- {position_notes_instruction}"""
    ...
    try:
        ...
        # Tool Use 응답에서 input 추출 (JSON 파싱 보장)
        result = None
        text_fallback = ""
        for c in content:
        ...
        # user_position 손익 계산은 코드에서 (모델에게 맡기지 않음 — 사실 정확성)
        user_position = None
        if avg_price_int:
        ...
        # 브리핑 로그 (캐시 미스만, archive/ai_briefings/ 로 후속 cron 이 옮김)
        _log_ai_call({
