...
AI_BRIEFING_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            "overall_verdict": {
                "type": "object",
                "description": "단기 1-2주 종합 신호 평가. 시장+기술+신뢰구간 종합.",
                "properties": {
                    "level": {
                        "type": "string",
...
- 시장 분위기·기술 지표·신뢰구간을 종합해 한 라벨로.
- 단정 표현 금지. "분위기 우세", "가능성" 같은 조건부 표현.
- summary 에 정렬 신호와 충돌 신호 균형 있게 명시.

다른 필드 금지 사항:
- 매매 권유 어휘·행동 동사 ("매수/매도하세요", "익절 고려" 등)
- 미래 예측 단정 ("오를 것이다")
- 데이터에 없는 정보 추측

출력 형식:
- 응답은 반드시 `report_stock_briefing` 도구 호출. 자유 텍스트 금지.
- 데이터 없는 필드는 null.
"""
...
def get_ai_analysis(code: str, client_ip: str = None, avg_price=None, shares=None) -> dict:
    ...
    cache_key = f"aib9:{code}:{now_kst().strftime('%Y-%m-%d')}{cache_suffix}"  # v9: max_tokens 2500 (1500은 뉴스 분류 잘림)
